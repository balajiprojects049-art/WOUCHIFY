...
import { getStorePathByName, storesData } from '../data/storesData'
...
function TopStoresSection() {
  ...
  const otherStores = storesData.slice(1)
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-5">
          {featuredStore && (
            <article className="col-span-2 row-span-2 rounded-2xl border border-line bg-cream p-4 transition-all duration-300 hover:-translate-y-2 sm:p-5">
              ...
              <button
                onClick={() => navigate(getStorePathByName(featuredStore.name))}
          ...
          {otherStores.map((store) => (
            <button
              key={store.name}
              onClick={() => navigate(getStorePathByName(store.name))}
