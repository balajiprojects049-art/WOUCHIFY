...
import { ChevronRight, Filter, Search, Tag, Sparkles, LayoutGrid, CheckSquare, Square, Store, Compass, CreditCard } from 'lucide-react'
...
export default function CategoryResults() {
  ...
  }, [categoryName]) // reset scroll and state when category changes

  // Classification Helpers
  const isStore = useMemo(() => {
    if (!categoryName) return false
    return Object.values(categoriesData.stores || {}).some(arr => 
      arr.some(item => item.name.toLowerCase() === categoryName.toLowerCase())
    )
  }, [categoryName])

  const isBrand = useMemo(() => {
    if (!categoryName) return false
    return Object.values(categoriesData.brands || {}).some(arr => 
      arr.some(item => item.name.toLowerCase() === categoryName.toLowerCase())
    )
  }, [categoryName])
  ...
  const isBank = useMemo(() => {
    if (!categoryName) return false
    return Object.values(categoriesData.banks || {}).some(arr => 
      arr.some(item => item.name.toLowerCase() === categoryName.toLowerCase())
    )
  }, [categoryName])

  // Fetch all items assigned to this category / store / brand / bank
  const categoryDeals = useMemo(() => {
    if (!categoryName) return []
    const catLower = categoryName.toLowerCase()
    
    return deals.filter(d => {
      if (getDealRemainingSeconds(d, nowMs) <= 0) return false
      
      if (isStore) {
        return d.store?.toLowerCase() === catLower || d.store?.toLowerCase().includes(catLower) || catLower.includes(d.store?.toLowerCase())
      }
      if (isBrand) {
        return d.brand?.toLowerCase() === catLower || 
               d.title?.toLowerCase().includes(catLower) || 
               d.description?.toLowerCase().includes(catLower) ||
               d.store?.toLowerCase() === catLower
      }
      if (isBank) {
        ...
        return searchWords.some(kw => 
          ...
          d.store?.toLowerCase().includes(kw) ||
          d.category?.toLowerCase().includes(kw)
        )
      }
      
      return d.category?.toLowerCase() === catLower
    })
  }, [deals, categoryName, nowMs, isStore, isBrand, isBank])

  const categoryLoot = useMemo(() => {
    if (!categoryName) return []
    const catLower = categoryName.toLowerCase()
  ...
  }, [lootDeals, categoryName, nowMs, isStore, isBrand, isBank])

  const categoryCoupons = useMemo(() => {
    if (!categoryName) return []
    const catLower = categoryName.toLowerCase()
    
    return coupons.filter(c => {
      if (isStore) {
        return c.store?.toLowerCase() === catLower || c.store?.toLowerCase().includes(catLower) || catLower.includes(c.store?.toLowerCase())
      }
      if (isBrand) {
        return c.brand?.toLowerCase() === catLower || 
      ...
      return c.category?.toLowerCase() === catLower
    })
  }, [coupons, categoryName, isStore, isBrand, isBank])

  const bankCards = useMemo(() => {
    if (!isBank || !categoryName) return []
    const catLower = categoryName.toLowerCase()
    const all = [...(creditCards?.shopping || []), ...(creditCards?.lifetime || [])]
    return all.filter(c => 
      c.bank?.toLowerCase().includes(catLower) ||
      catLower.includes(c.bank?.toLowerCase())
    )
  }, [creditCards, isBank, categoryName])
  ...
  const visibleCategories = useMemo(() => {
    if (isBank) {
      const list = []
      Object.values(categoriesData.banks || {}).forEach(arr => list.push(...arr))
      return list.sort((a, b) => a.name.localeCompare(b.name)).filter(c => c.name.toLowerCase().includes(categorySearch.toLowerCase()))
    }
    return allCategories.filter(c => c.name.toLowerCase().includes(categorySearch.toLowerCase()))
  }, [isBank, allCategories, categorySearch])
  ...
  const totalCount = useMemo(() => {
    if (isBank) {
      return bankCards.length + filteredDeals.length + filteredCoupons.length
    }
    return filteredDeals.length + filteredLoot.length + filteredCoupons.length
  }, [isBank, bankCards, filteredDeals, filteredLoot, filteredCoupons])
    ...
    <div className="min-h-screen bg-cream font-sans transition-colors duration-500">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cream border border-line mb-3 shadow-sm">
                 {isBank ? <CreditCard className="w-3.5 h-3.5 text-gold" /> : <Tag className="w-3.5 h-3.5 text-gold" />}
                 <span className="text-[10px] font-black text-ink uppercase tracking-[0.2em]">{isBank ? 'Bank Partner Results' : 'Category Results'}</span>
              </div>
            ...
            <div className="flex items-center gap-1 p-1 bg-surface rounded-2xl border border-line shadow-sm overflow-x-auto no-scrollbar">
               {(isBank ? [
                 ...
                 { id: 'cards', label: 'Credit Cards', count: bankCards.length },
                 { id: 'offers', label: 'Offers', count: filteredDeals.length },
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-10 flex flex-col lg:flex-row gap-8">
         ...
         <aside className="w-full lg:w-64 shrink-0 space-y-6">
            {/* Store Filter */}
            {availableStores.length > 0 && !isBank && (
            ...
            <div className="bg-surface rounded-2xl border border-line p-5 shadow-sm">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-bold text-ink uppercase tracking-wider">{isBank ? 'Other Banks' : 'Related Categories'}</h3>
              </div>
         ...
         <div className="flex-grow min-w-0">
            {isBank ? (
              ...
              <>
                {(tab === 'all' || tab === 'cards') && bankCards.length > 0 && (
                  <section className="mb-12">
                     <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-extrabold text-ink tracking-tight">Credit Cards & Benefits</h2>
                     </div>
                ...
                {(tab === 'all' || tab === 'offers') && filteredDeals.length > 0 && (
                  <section className="mb-12">
                     <div className="flex items-center gap-3 mb-4">
                        <div className="w-8 h-8 rounded-lg bg-red-500/10 flex items-center justify-center text-red-500">
                           <Sparkles className="w-4 h-4" />
                        </div>
                        <h2 className="text-xl font-extrabold tracking-tight text-ink">Cashback & Partnership Offers</h2>
                     </div>
                ...
                {(tab === 'all' || tab === 'coupons') && filteredCoupons.length > 0 && (
                  <section className="mb-12">
                     <div className="flex items-center gap-3 mb-6">
                        ...
                        <h2 className="text-xl font-extrabold tracking-tight text-blue-500">Bank Promo Coupons</h2>
                     </div>
