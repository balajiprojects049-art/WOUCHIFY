...
const EMPTY = {
  ...
  expiry: '', minOrder: '', success: '', badge: 'NEW', active: true,
}
const CATEGORIES = ['All Categories', 'Electronics', 'Fashion & Lifestyle', 'Food Delivery', 'Travel', 'Beauty & Health', 'Shopping', 'Fashion']
          ...
          <div className="rounded-2xl p-5" style={cardStyle}>
            ...
            <div className="rounded-xl p-5" style={{ background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.12)' }}>
              <div className="flex items-center gap-3 mb-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl text-sm font-black" style={{ background: 'rgba(0,212,126,0.12)', color: G }}>
                  {(form.store || 'S').charAt(0).toUpperCase()}
                </div>
                <div>
      ...
      <div style={tableWrapStyle}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr style={thStyle}>
                {['Store', 'Code', 'Discount', 'Status', 'Actions'].map((h, i) => (
                  <th key={h} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-widest text-white/30 ${i === 4 ? 'text-right' : 'text-left'} ${i === 2 ? 'hidden sm:table-cell' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
