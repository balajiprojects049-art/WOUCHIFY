...
const EMPTY_DEAL = {
  ...
  type: 'deal', price: '', priceValue: 0, usageCount: '', expiresInSeconds: 7200,
  successRate: 90, expiry: '', badge: 'HOT', code: '', image: '', description: '',
  terms: '', steps: '', highlights: '', createdAt: new Date().toISOString(),
}
const CATEGORIES = ['Electronics', 'Fashion', 'Accessories', 'Travel', 'Lifestyle', 'Food', 'Beauty']
const BADGES = ['HOT', 'TRENDING', 'EXCLUSIVE', 'LIMITED', 'COUPON', 'TRAVEL', 'NEW', 'FLASH']
const TYPES = ['deal', 'coupon']
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select {...selectProps} value={form.type} onChange={e => set('type', e.target.value)}>
                  {TYPES.map(t => <option key={t} value={t} className="bg-[#0C1018]">{t === 'deal' ? 'Deal' : 'Coupon'}</option>)}
                </select>
              </div>
              <div>
          ...
          <div className="rounded-2xl p-5" style={cardStyle}>
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                ...
                { lbl: 'Usage Count', key: 'usageCount', ph: '2.9k' },
    ...
    <AdminLayout title="Manage Deals">
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative sm:max-w-xs w-full">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search deals..." className={searchInpCls} style={searchInpStyle} />
        </div>
        <button onClick={() => { setMode('add'); setEditing(null) }} className="shrink-0 rounded-xl px-6 py-2.5 text-sm font-black transition-all hover:opacity-90" style={btnPrimary}>+ Add New Deal</button>
      </div>
      ...
      <div style={tableWrapStyle}>
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr style={thStyle}>
                {['Deal', 'Store', 'Badge', 'Price', 'Actions'].map((h, i) => (
                  <th key={h} className={`px-5 py-3.5 text-[10px] font-black uppercase tracking-widest text-white/30 ${i === 4 ? 'text-right' : 'text-left'} ${i === 1 ? 'hidden sm:table-cell' : ''} ${i === 2 ? 'hidden md:table-cell' : ''}`}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-white/30">No deals yet. Click "+ Add New Deal" to get started.</td></tr>}
              {filtered.map((deal) => (
                <tr key={deal.slug} className="transition-colors" style={trBorderStyle}
                ...
                >
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      {deal.image
                        ? <img src={deal.image} alt="" className="h-9 w-9 rounded-lg object-cover shrink-0" style={{ border: '1px solid rgba(255,255,255,0.10)' }} onError={e => e.target.style.display = 'none'} />
                        : <div className="flex h-9 w-9 items-center justify-center rounded-lg shrink-0" style={{ background: 'rgba(0,212,126,0.10)', color: G, fontSize: '0.8rem', fontWeight: 900 }}>D</div>
                      }
                      <div>
                        <p className="text-sm font-semibold text-white leading-tight">{deal.title}</p>
                        <p className="text-[11px] text-white/30">{deal.category}</p>
                      </div>
                    </div>
                  </td>
                  <td className="hidden px-5 py-4 text-sm text-white/50 sm:table-cell">{deal.store}</td>
                  <td className="px-5 py-4 hidden md:table-cell">
                    <span className="rounded-full px-2.5 py-1 text-[10px] font-black" style={badgePillStyle}>{deal.badge}</span>
                  </td>
      ...
      <p className="mt-3 text-xs text-white/30">{filtered.length} of {deals.length} deals</p>
    </AdminLayout>
  )
}
