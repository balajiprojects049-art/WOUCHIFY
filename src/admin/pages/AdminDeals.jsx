...
function DealForm({ initial, onSave, onCancel, role, stores }) {
  ...
  const [form, setForm] = useState(() => {
    ...
    return { ...EMPTY_DEAL, ...initial, expiresAt: d.toISOString().slice(0, 16), images: initImages, status: isEditor ? 'Draft' : initial.status || 'Active' }
  })
  ...
  const set = (k, v) => setForm(p => {
    ...
    const calcFields = ['price', 'originalPrice', 'priceValue', 'discountValue']
    if (calcFields.includes(k)) {
      ...
      const mrp = parseNum(k === 'originalPrice' ? v : p.originalPrice)
      
      if (k === 'priceValue' || k === 'originalPrice') {
        const curr = parseNum(k === 'priceValue' ? v : p.priceValue)
        
        if (k === 'priceValue') next.price = curr > 0 ? `₹${curr.toLocaleString('en-IN')}` : ''

        if (mrp > 0 && curr > 0 && mrp > curr) {
          const percent = Math.round(((mrp - curr) / mrp) * 100)
          next.discountValue = percent
          next.discountLabel = `Save ${percent}%`
        }
      } else if (k === 'discountValue') {
        const percent = parseNum(v)
        if (mrp > 0 && percent >= 0 && percent <= 100) {
          const curr = Math.round(mrp - (mrp * (percent / 100)))
          next.priceValue = curr
          next.price = curr > 0 ? `₹${curr.toLocaleString('en-IN')}` : ''
          next.discountLabel = percent > 0 ? `Save ${percent}%` : ''
        }
      } else if (k === 'price') {
        const curr = parseNum(v)
        next.priceValue = curr || ''
        if (mrp > 0 && curr > 0 && mrp > curr) {
          const percent = Math.round(((mrp - curr) / mrp) * 100)
          next.discountValue = percent
